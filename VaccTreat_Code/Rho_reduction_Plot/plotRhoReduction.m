...
ax(3) = axes('Position',  [leftmargin+2*plotwidth+2*columnspace, bottommargin+1*plotheight+1*rowspace, plotwidth, plotheight]); 

%middle row
ax(4) = axes('Position',  [leftmargin,                           bottommargin+0*plotheight+0*rowspace, plotwidth, plotheight]);
ax(5) = axes('Position',  [leftmargin+plotwidth+columnspace,     bottommargin+0*plotheight+0*rowspace, plotwidth, plotheight]);
ax(6) = axes('Position',  [leftmargin+2*plotwidth+2*columnspace, bottommargin+0*plotheight+0*rowspace, plotwidth, plotheight]); 
...
%            interventioncases interventiondeaths interventiontxdoses interventionmaxhcwratio;
    filename = sprintf('VaccTreatmentStochResults_delay183_rho%g', delays(index));
    load(filename);
...
%     
   ...
   % xlabel('Treatment efficacy, \epsilon_T', 'FontSize', labelsize, 'FontName', 'Palatino')
    ...
    switch index
        case 1
            title('a) \rho = 30%', 'FontSize', titlesize, 'FontName', 'Palatino')
        case 2
            title('b) \rho = 60%', 'FontSize', titlesize, 'FontName', 'Palatino')
        case 3
            title('c) \rho = 100%', 'FontSize', titlesize, 'FontName', 'Palatino')
    end
...
%     end
   ...
   % plot(h4, xaxis, interventionmaxhcwratio_mean);
    ...
    if index==1 
        text(-0.05,84.619, {'Maximum Ebola', 'patients per HCW'}, ...
            'Rotation', 90, 'FontName', 'Palatino', 'FontSize', titlesize,...
            'HorizontalAlignment', 'Center'); 
