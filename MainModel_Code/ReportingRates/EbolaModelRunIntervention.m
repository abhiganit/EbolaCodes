...
incout = cellfun( @(a) diff(a), intervention_cases, 'UniformOutput', false);
%incout = cellfun( @(o1, o2) [o1, o2], postintervention_cases, intervention_cases, 'UniformOutput', false);
...
%     
%     
    % we're only interested in these strategies
    cp(1+1,:) = [0, 1, 1, 0, 0, 1];  %transmission reduction (hospital+community) + hygienic burial (hospital cases) 
    cp(2+1,:) = [0, 0, 0, 0, 1, 1];  %hygienic burial (hospital+community cases)
    cp(3+1,:) = [1, 0, 0, 1, 0, 0];  %isolation (hospital cases) + contact follow-up
